...
class QChatModel extends Croquet.Model {
    init() {
        ...
        this.subscribe(this.sessionId, 'peer-stream-subscriptions', this.onPeerStreamSubscriptions);
    ...
    // has the peer been seen?  once true, this will never be reset.
    ...
    peerSnapshotForId(viewId) {
        ...
        const snap = { ...peer };
        snap.subscribed = snap.subscribed ? [ ...snap.subscribed ] : [];
    ...
    onViewJoin(viewId) {
        ...
        this.publish(viewId, 'local-view-join');
    }
    ...
    onViewExit(viewId) {
        ...
        const peer = this.peerDict[viewId];
        // keep just the peerIndex, as explained above.
        this.peerDict[viewId] = { peerIndex: peer.peerIndex };

        if (peer.raisingHand) this.onPeerHand({ viewId, raisingHand: false });
        this.publish(this.sessionId, 'on-peer-exit', viewId);
    }

    onChatPeerDetails({viewId, nickname, initials, viewColor, agent}) {
        ...
        if (!peer) {
            ...
            // if this brings the number of peers to 3, force the
            // views out of solo (so everyone is visible) and set
            // the new arrival as the active peer.
            const numPeers = this.identifiedPeers().length;
            if (numPeers === 3) {
                this.onSetSolo(false);
                this.onSetActivePeer(viewId, true); // and publish
            } else if (numPeers === 1) {
    ...
    onPeerStreamSubscriptions({viewId, subscribed}) {
        ...
        if (peer) {
            ...
            this.publish(this.sessionId, 'on-peer-stream-subscriptions', { viewId, subscribed });
        }
    }
