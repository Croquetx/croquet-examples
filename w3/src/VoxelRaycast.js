...
// Returns a list of voxel XYZ's along the ray.  It is clipped so it only includes voxels
// that are actually in the world tile.

function VoxelIsValid(xyz) {
    return Voxels.isValid(...xyz);
}

export function FilteredVoxelRaycast(start, aim) {
    return VoxelRaycast(start, aim).filter(VoxelIsValid);
}
